...
(() => {
  ...
  // ============================================================
  const MASCOT_ASSETS = {
    base: 'assets/img/mascots/about_mascots/',
    ext:  'webp',
    files: {
  ...
  // --- Reveal stagger (R10) ---
  // Any [data-stagger] container cascades its direct children in: each child
  // gets an incremental --reveal-delay, and is made a .reveal if it isn't one,
  // so the group eases in one-by-one with the canonical --a28-ease-out.
  const STAGGER_STEP = 80; // ms between children
  document.querySelectorAll('[data-stagger]').forEach(group => {
    const step = parseInt(group.getAttribute('data-stagger'), 10) || STAGGER_STEP;
  ...
  // Capabilities hero heading — same line-rise reveal as work/about (R4).
  const capHl = document.getElementById('cap-heading');
  if (capHl) requestAnimationFrame(() => setTimeout(() => capHl.classList.add('is-revealed'), 220));
  ...
  if (heroCapy && heroCapyImg &&
    ...
    for (let k = 0; k < FRAME_COUNT; k++) {
      ...
      const img = new Image();
      img.src = 'assets/frames/beachcapyspin/' + String(n).padStart(4, '0') + '.webp';
  ...
  // --- "What we build" cards ---
  // (3.3 item 5) These are now deep-links to the matching capability card on
  // capabilities.html, so click/Enter navigate. The photo overlay still reveals
  // on hover/focus via CSS — the old tap-to-pin toggle was removed because it
  // hijacked the click and preventDefault-ed Enter, blocking link navigation.
  ...
  // --- About (R6): "Classified dossier" files — click a cover to declassify.
  // Each file toggles independently; updates the lock glyph + cue copy. ---
  document.querySelectorAll('.gov-file').forEach((file) => {
    const cover = file.querySelector('.gov-file__cover');
    if (!cover) return;
    const lock = cover.querySelector('.gov-file__lock');
    const cueText = cover.querySelector('.gov-file__cue-text');
    cover.addEventListener('click', () => {
      const open = file.classList.toggle('is-open');
      cover.setAttribute('aria-expanded', open ? 'true' : 'false');
      if (lock) lock.textContent = open ? 'lock_open' : 'lock';
      if (cueText) cueText.textContent = open ? 'Declassified — click to seal' : 'Click to declassify';
    });
  });
  ...
  // Government route ports the same markup as #gov-mascot-stage (recoloured
  // via .gov-skin); only one route renders at a time, so resolve the stage
  // for the active theme and run the identical state machine on it.
  const mascotTheme = document.documentElement.dataset.theme;
  const mascotStage = document.getElementById(
    mascotTheme === 'government' ? 'gov-mascot-stage' : 'ent-mascot-stage'
  );
  if (mascotStage && (mascotTheme === 'enterprise' || mascotTheme === 'government')) {
  ...
  // --- Government hero: scroll-jacked marquee + 2 spotlight face reveals ---
  // Dark "mission" hero. Vertical scroll pans the tagline left while two
  // spotlights sweep in at fixed scroll points to reveal the suited mascots,
  // then vanish. Desktop / no-reduced-motion only (matchMedia); mobile and
  // reduced-motion get the static, naturally-wrapped hero from CSS.
  const govLine = document.getElementById('gov-hero-line');
  const govSection = document.getElementById('gov-hero');
  const govCapy = document.getElementById('gov-spot-capy');
  const govOyen = document.getElementById('gov-spot-oyen');
  if (govLine && govSection && govCapy && govOyen &&
      document.documentElement.dataset.theme === 'government' &&
      window.gsap && window.ScrollTrigger) {
    gsap.registerPlugin(ScrollTrigger);

    const TEXT_END_FRACTION = 0.75;
    const travel = () => Math.max(0, govLine.scrollWidth - window.innerWidth * TEXT_END_FRACTION);

    const mm = gsap.matchMedia();
    mm.add('(min-width: 768px) and (prefers-reduced-motion: no-preference)', () => {
      govSection.classList.add('is-ready');
      gsap.set([govCapy, govOyen], { autoAlpha: 0, xPercent: -50, transformOrigin: '50% 50%' });
      ...
      // Marquee pans the full width across the whole scroll range.
      tl.to(govLine, { x: () => -travel(), ease: 'none', duration: 1 }, 0);

      // Stage-spotlight bob: each mascot rises OUT of the tagline, turning its
      // angle as it lifts, then sinks back DOWN and dissolves into the line.
      // Horizontally each circle is LOCKED to the tagline — it pans left by the
      // same fraction of travel() over its window as the line does over that
      // same stretch of scroll, so it rides with the words rather than drifting
      // on its own. The vertical bob (yPercent) and rotation are the only motion
      // that's truly its own. yPercent is relative to the circle's own height.

      // R3 — clean 3-phase symmetric arc so the IN and OUT easing read evenly.
      // Each face: (1) rises in from the line decelerating to its apex
      // [power2.out], (2) holds with a gentle equal-velocity bob [sine.inOut],
      // (3) sinks back out accelerating away [power2.in]. The three phases are
      // EQUAL length and butt-join at zero velocity, so there are no snaps and
      // the in/out feel mirror each other. Horizontally each face stays locked
      // to the panning tagline (its x delta == the line's pan over its window).

      // Techybara — window 0.10 → 0.52 (rises UP, top-edge of the line).
      tl.fromTo(govCapy, { x: () => travel() * 0.20 }, { x: () => -travel() * 0.20, ease: 'none', duration: 0.42 }, 0.10);
      // (1) rise in
      tl.fromTo(govCapy,
        { autoAlpha: 0, yPercent: 30, scale: 0.80, rotation: -11 },
        { autoAlpha: 1, yPercent: -40, scale: 1, rotation: 3, ease: 'power2.out', duration: 0.14 }, 0.10);
      // (2) hold + gentle bob
      tl.to(govCapy, { yPercent: -33, rotation: -2, ease: 'sine.inOut', duration: 0.14 }, 0.24);
      // (3) sink out
      tl.to(govCapy, { autoAlpha: 0, yPercent: 26, scale: 0.80, rotation: -12, ease: 'power2.in', duration: 0.14 }, 0.38);

      // Oyen — window 0.52 → 0.94, mirrored (dips DOWN, lower edge of the line).
      tl.fromTo(govOyen, { x: () => travel() * 0.20 }, { x: () => -travel() * 0.20, ease: 'none', duration: 0.42 }, 0.52);
      // (1) drop in
      tl.fromTo(govOyen,
        { autoAlpha: 0, yPercent: -30, scale: 0.80, rotation: 11 },
        { autoAlpha: 1, yPercent: 40, scale: 1, rotation: -3, ease: 'power2.out', duration: 0.14 }, 0.52);
      // (2) hold + gentle bob
      tl.to(govOyen, { yPercent: 33, rotation: 2, ease: 'sine.inOut', duration: 0.14 }, 0.66);
      // (3) lift out
      tl.to(govOyen, { autoAlpha: 0, yPercent: -26, scale: 0.80, rotation: 12, ease: 'power2.in', duration: 0.14 }, 0.80);
      ...
      return () => {
        gsap.set([govLine, govCapy, govOyen], { clearProps: 'transform,opacity,visibility' });
      };
    });
  }
